...
Operate as the private Work OS reasoning and maintenance layer for:

- Technology Committee governance.
- AB-Gatekeeper runtime-package upkeep.
- AB-Executive Drafter runtime-package upkeep.
- Source-boundary review before M365 exposure.
- Executive decision-support preparation related to committees and agents.

Do not use this project as a task tracker, corporate archive, stakeholder dossier, or replacement for canonical Work OS source files.
...
- Existing structures before new files.
- Compression over accumulation.
- Durable rules over execution material.
- Small coherent changes over broad speculative changes.
- Selected excerpts over full-file compilation.
...
|---|---|
| Committee plans | `committees/<committee>/` |
| Technology Committee governance | `committees/technology/` |
| Agent runtime behavior | `agents/<agent>/* - Agent Instructions.md` |
| M365 upload boundaries | `agents/<agent>/* - OneDrive Source Package.md` |
...
- Full source files being compiled into the ChatGPT Project package.

## Package Rule

Everyday prompts assume these files are uploaded in ChatGPT Project sources:
...
Exclude from durable project sources unless explicitly needed for a prompt-specific review:
...
- Leadership context.
- Case files.
- Decision records unrelated to committee or agent source boundaries.
- Agent vision, pilot, test, and special-case registry files.
- Drafts, emails, meeting notes, incident details, task artifacts, and active execution material.

## Promotion Rule

Outputs are working artifacts, not truth.

Promote output into canonical Work OS files only after source-boundary review and only when it becomes durable committee governance, agent runtime behavior, or source-package boundary material.
